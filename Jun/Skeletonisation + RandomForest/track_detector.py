...
def skeletonise_frame(frame, bg_subtractor=None):
    """
    Preprocess a full frame into its skeletonised binary representation.

    Runs the same detection preprocessing used for tracks (optional background
    subtraction, CLAHE, denoise, blur, threshold, morphology) and then thins
    the whole binary mask to a 1-pixel-wide skeleton.

    Parameters
    ----------
    frame : np.ndarray
        BGR video frame (or grayscale).
    bg_subtractor : cv2.BackgroundSubtractor or None
        Optional background subtractor used to isolate moving tracks.

    Returns
    -------
    skeleton_frame : np.ndarray (uint8)
        Full-frame skeleton image (0 or 255), same H×W as the input frame.
    """
